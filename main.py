...
def generate_replay_code_server(buf_size, server_name=None):
    ...
    calls = [
        ...
        RegMR(pd_addr="PD", buf="bufs[0]", length=buf_size, mr_addr="MR",
              flags="IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_WRITE", ctx=ctx),
        CreateQP(pd_addr="PD", qp_addr="QP", cq_addr = "CQ", qp_type="IBV_QPT_RC", cap_params={
            "max_send_wr": 1,
            "max_recv_wr": 1,
            "max_send_sge": 1,
        ...
        ExportQPInfo(qp_addr="QP", mr_addr="MR"),  # Export QP info
        SockSyncData(),  # Synchronize connection data over socket
        ImportQPInfo(0),  # Import QP info from remote
        # modify QP to INIT state
        ModifyQP(qp_addr="QP", state="IBV_QPS_INIT"),
        # PostRecv(qp_addr="QP", mr_addr = "MR", wr_id="0"),  # Post a receive request
        # modify QP to RTR state
        # 这是一个问题，remote_qpn和dlid应该从远端获取，暂时写死为1和0
        ModifyQPToRTR(qp_addr="QP", qpn = 0),
        # modify QP to RTS state
        ModifyQPToRTS(qp_addr="QP"),
        # SockSyncDummy(),  # Dummy sync, no actual data transfer
        # connect_qp end
        # post_send start
        PostSend(qp_addr="QP", mr_addr = "MR", wr_id="1", opcode="IBV_WR_SEND"),
        PollCQ(cq_addr="CQ"),
        # post_send end
