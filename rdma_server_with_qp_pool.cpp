// 修改后的 rdma_server_with_qp_pool.c
...
{
    printf("Posting all receives to QP %d\n", qp->qp_num);
    for (int i = 0; i < RECV_POOL_SIZE; ++i)
    {
        struct ibv_sge sge = {
        ...
        {
            fprintf(stderr, "Failed to post receive work request for slot %d\n", i);
        }
    }
}
...
char *serialize_metadata_global(struct metadata_global *meta)
{
    ...
    cJSON_AddStringToObject(json, "role", "server");
...
const char *deserialize_metadata(const char *json_str)
{
    ...
    { // will not trigger on both sides
    ...
    { // will not trigger on server-side
        ...
        cJSON_Delete(json);
        return result;
        // fprintf(stderr, "MR metadata deserialization not implemented yet.\n");
        // cJSON_Delete(json);
        // return NULL;
    }
    else if (strcmp(type, "pair") == 0)
    {
        ...
        int local_qpn = meta.local_qpn;
        int remote_qpn = meta.remote_qpn;
        ...
        printf("what's wrong with this? %d\n", local_index);

        post_all_recvs(&qp_pool[local_index].recv_pool, qp_pool[local_index].qp); // 重新发布接收请求
...
int main()
{
    ...
    receive_metadata_from_controller(sockfd); // 接受 global_metadata

    receive_metadata_from_controller(sockfd); // 接受 pair_metadata
