...
def render(seq):
    ...
    tpl = env.get_template("client.cpp.j2")

    ctx = CodeGenContext()

    # 给每个 verb 编号，并 apply（登记事件）
    for i, v in enumerate(seq):
        setattr(v, "_seq_index", i)
        v.apply(ctx)

    # 生成代码：before(i) → verb(i) → after(i)
    body = []
    for i, v in enumerate(seq):
        if ctx.events_before[i]:
            body.append("\n".join(ctx.events_before[i]))
        body.append(v.generate_c(ctx))
        if ctx.events_after[i]:
            body.append("\n".join(ctx.events_after[i]))
