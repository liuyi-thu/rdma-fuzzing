...
for func in func_list:
    ...
    second_message = f'以上的文件里提供了很多写好的代码，可以为你接下来的任务提供参考。我现在想要生成一个{func}对应的代码，能够加入到verbs.py里面，生成{func}的C代码。'
    with open(f'/home/liuyi/router-lab/fuzzing-rdma/rdma-fuzzing/prompts/{func}.prompt.txt', 'r') as f:
        prompt = f.read()
        second_message += prompt
    # print(second_message)
    third_message = '之前我让你已经帮我生成了5个verbs的代码，请你参考，从中选择最好的，并输出给我，分别是：'
    for i in range(1, 6):
        with open(f'/home/liuyi/router-lab/fuzzing-rdma/rdma-fuzzing/responses_00{i}/{func}.response.txt', 'r') as f:
            response = f.read()
            third_message += f'\n{i}. {response}'
    
    
    # print(first_message + second_message + third_message + fourth_message)
    # exit(0)
    completion = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "你是一个代码助手。你会根据用户提供的代码和提示，生成相应的Python代码。"},
            {
                "role": "user",
