...
struct ibv_recv_wr recv_wr_srq0_next;
struct ibv_sge recv_wr_srq0_next_sg_list[3];
...
int main(int argc, char **argv)
{
    ...
    printf("[24] PostSRQRecv(srq=srq0, wr_obj=IbvRecvWR{sg_list=[IbvSge x1: IbvSge{addr=(uint64_t)mr0->addr, length=mr0->length, lkey=mr0->lkey}], wr_id=3676860885, next=IbvRecvWR{num_sge=3, sg_list=[IbvSge x3: IbvSge{addr=\u2205, length=\u2205, lkey=\u2205}], wr_id=3909861508425510039}}, wr_var=recv_wr_srq0, bad_wr_var=bad_recv_wr_srq0) start.\n");

    /* ibv_post_srq_recv (fuzz-friendly guarded) */
    IF_OK_PTR(srq0, {
        IF_OK_PTR(mr0, {
            memset(&recv_wr_srq0, 0, sizeof(recv_wr_srq0));
            recv_wr_srq0.wr_id = 3676860885;
            ...
            memset(&recv_wr_srq0_next, 0, sizeof(recv_wr_srq0_next));
            recv_wr_srq0_next.wr_id = 3909861508425510039;

            memset(&recv_wr_srq0_next_sg_list[0], 0, sizeof(recv_wr_srq0_next_sg_list[0]));

            memset(&recv_wr_srq0_next_sg_list[1], 0, sizeof(recv_wr_srq0_next_sg_list[1]));

            memset(&recv_wr_srq0_next_sg_list[2], 0, sizeof(recv_wr_srq0_next_sg_list[2]));
            recv_wr_srq0_next.sg_list = recv_wr_srq0_next_sg_list;
            recv_wr_srq0_next.num_sge = 300;
            recv_wr_srq0_next.next = NULL;
            recv_wr_srq0.next = &recv_wr_srq0_next;
