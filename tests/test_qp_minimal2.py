...
# Import from the user's lib/ package layout
...
def test_modify_qp_minimal_apply_and_codegen():
    ...
    create = verbs.CreateQP(pd="pd0", qp="qp0", init_attr_obj=init_attr)
    create.apply(ctx)

    attr = IbvQPAttr(qp_state="IBV_QPS_RTS", cur_qp_state="IBV_QPS_INIT", path_mtu="IBV_MTU_1024")
    mod = verbs.ModifyQP(qp="qp0", attr_obj=attr, attr_mask="IBV_QP_STATE")
    mod.apply(ctx)
    assert {"type": "qp", "name": "qp0", "position": "qp"} in getattr(mod, "required_resources", [])
    code = mod.generate_c(ctx)
    assert_contains(code, "ibv_modify_qp")


def test_create_qp_cap_mutation_smoke():
    ctx = FakeCtx()
    _mk_pd_cq(ctx)

    cap = IbvQPCap(max_send_wr=1, max_recv_wr=1, max_send_sge=1, max_recv_sge=1)
    # minimal mutation: increment counts within a sane range
    if hasattr(cap, "max_send_wr"):
        cap.max_send_wr = 2
    if hasattr(cap, "max_recv_wr"):
        cap.max_recv_wr = 2
    if hasattr(cap, "max_send_sge"):
        cap.max_send_sge = 2
    if hasattr(cap, "max_recv_sge"):
        cap.max_recv_sge = 2
