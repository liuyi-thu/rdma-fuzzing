...
class AdviseMR(VerbCall):
    ...
    def __init__(
        ...
        self.num_sge = num_sge if num_sge is not None else len(self.sg_list)  # list的变异比较麻烦，得针对处理，待定
...
class AttachMcast(VerbCall):
    ...
    def __init__(self, qp: str = None, gid: str = None, lid: int = None):  # 注意gid需要是变量，不然无法传参
...
class BindMW(VerbCall):
    ...
    def __init__(self, qp: str = None, mw: str = None, mw_bind_var: str = None, mw_bind_obj: IbvMwBind = None):
        """
        qp: QP变量名
        mw: MW变量名
...
class CreateAH(VerbCall):
    ...
    def __init__(self, pd: str = None, attr_var: str = None, ah: str = None, attr_obj: "IbvAHAttr" = None):
        self.pd = ResourceValue(resource_type="pd", value=pd) if pd else ResourceValue(resource_type="pd", value="pd")
        # 默认生成 ah_attr_<pd>
        self.attr_var = ConstantValue(attr_var or f"ah_attr_{self.pd.value}")  # 结构体变量名
        # self.ah = ConstantValue(ah or f"ah_{self.pd.value}")  # 返回的 ibv_ah* 变量名
        self.ah = (
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            ...
            self.required_resources.append({"type": "pd", "name": self.pd.value, "position": "pd"})  # 记录需要的资源
            self.allocated_resources.append(("ah", self.ah.value))  # 记录已分配的资源
...
class CreateAHFromWC(VerbCall):
    ...
    def __init__(self, pd: str = None, wc: str = None, grh: str = None, port_num: int = None, ah: str = None):
        self.pd = ResourceValue(resource_type="pd", value=pd) if pd else ResourceValue(resource_type="pd", value="pd")
...
class CreateCompChannel(VerbCall):
    ...
    def __init__(self, channel: str = None):
        self.channel = (
            ResourceValue(resource_type="comp_channel", value=channel)
            if channel
            else ResourceValue(resource_type="comp_channel", value="comp_channel")
        )
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            ...
            self.tracker.create("comp_channel", self.channel.value)
            self.allocated_resources.append(("comp_channel", self.channel.value))  # 记录已分配的资源
...
class CreateCQ(VerbCall):
    ...
    def __init__(
        self, cqe: int = 32, cq_context: str = "NULL", channel: str = "NULL", comp_vector: int = 0, cq: str = None
    ):
        # 存疑，channel是否需要关联
        self.cqe = IntValue(cqe)  # Completion queue entries, default is 32
        # CQ context, default is "NULL"
        self.cq_context = ConstantValue(cq_context)
        # Completion channel, default is "NULL"
        self.channel = ConstantValue(channel)
        # Completion vector, default is 0
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            ...
            self.allocated_resources.append(("cq", self.cq.value))  # 记录已分配的资源
...
class CreateCQEx(VerbCall):
    ...
    def __init__(
        ...
        self.cq_ex = ResourceValue(resource_type="cq_ex", value=cq_ex)  # CQ_EX变量名
        # 默认生成 cq_attr_<cq_var>
        # CQ_EX attribute variable name
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            ...
            self.allocated_resources.append(("cq_ex", self.cq_ex.value))  # 记录已分配的资源
...
class CreateFlow(VerbCall):
    ...
    def __init__(
        self, qp: str = None, flow: str = None, flow_attr_var: str = None, flow_attr_obj: "IbvFlowAttr" = None
    ):
        self.qp = ResourceValue(resource_type="qp", value=qp) if qp else ResourceValue(resource_type="qp", value="qp")
        self.flow = (
...
class CreateSRQEx(VerbCall):
    ...
    def __init__(
        self, ctx_name: str = None, srq: str = None, srq_attr_var: str = None, srq_attr_obj: "IbvSrqInitAttrEx" = None
    ):
        # IBV context variable name, default is "ctx"
        self.ctx_name = ConstantValue(ctx_name or "ctx")
        self.srq = (
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            ...
            self.allocated_resources.append(("srq_ex", self.srq.value))  # 记录已分配的资源
...
class CreateWQ(VerbCall):
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            ...
            self.allocated_resources.append(("wq", self.wq.value))  # 记录已分配的资源
...
class DeallocMW(VerbCall):
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            ...
            # Ensure MW is deallocated
            self.tracker.destroy("mw", self.mw.value)
            self.required_resources.append({"type": "mw", "name": self.mw.value, "position": "mw"})  # 记录需要的资源
...
class DestroyCompChannel(VerbCall):
    ...
    MUTABLE_FIELDS = ["comp_channel"]

    def __init__(self, comp_channel: str = None):
        self.comp_channel = (
            ResourceValue(resource_type="comp_channel", value=comp_channel)
            if comp_channel
            else ResourceValue(resource_type="comp_channel", value="comp_channel")
        )  # 默认生成 comp_channel
        self.tracker = None
    ...
    def apply(self, ctx: CodeGenContext):
        ...
        if self.tracker:
            # Register the completion channel address in the tracker
            self.tracker.use("comp_channel", self.comp_channel.value)
            self.tracker.destroy("comp_channel", self.comp_channel.value)
            self.required_resources.append(
                {"type": "comp_channel", "name": self.comp_channel.value, "position": "comp_channel"}
            )  # 记录需要的资源
...
class ImportPD(VerbCall):
    ...
    def __init__(self, pd: str = None, pd_handle: int = None):
        ...
        # Default to 0 if not provided
        self.pd_handle = IntValue(pd_handle or 0)
...
class MemcpyFromDM(VerbCall):
    ...
    def __init__(self, host: str = None, dm: str = None, dm_offset: int = None, length: int = None):
...
class MemcpyToDM(VerbCall):
    ...
    def __init__(self, dm: str = None, dm_offset: int = None, host: str = None, length: int = None):
        self.dm = (
            ResourceValue(resource_type="dm", value=dm) if dm else ResourceValue(resource_type="dm", value="dm")
        )  # Device memory address
        # Offset in the device memory
...
class ModifyCQ(VerbCall):
    ...
    MUTABLE_FIELDS = ["cq", "attr_object", "attr_var"]

    def __init__(self, cq: str = None, attr_object: IbvModifyCQAttr = None, attr_var: str = None):
        self.cq = (
            ResourceValue(resource_type="cq", value=cq) if cq else ResourceValue(resource_type="cq", value="cq")
        )  # CQ address
...
class ModifyQP(VerbCall):
    ...
    def __init__(self, qp: str = None, attr: IbvQPAttr = None, attr_mask: str = None):
        self.qp = (
            ResourceValue(resource_type="qp", value=qp) if qp else ResourceValue(resource_type="qp", value="qp")
        )  # QP address
        self.attr = attr
...
class ModifyQPRateLimit(VerbCall):
    ...
    def __init__(self, qp: str = None, attr_var: str = None, attr_obj: "IbvQPRateLimitAttr" = None):
        self.qp = (
            ResourceValue(resource_type="qp", value=qp) if qp else ResourceValue(resource_type="qp", value="qp")
        )  # QP address
...
class ModifySRQ(VerbCall):
    ...
    def __init__(self, srq: str = None, attr_var: str = None, attr_obj: "IbvSrqAttr" = None, attr_mask: int = 0):
        self.srq = (
            ResourceValue(resource_type="srq", value=srq) if srq else ResourceValue(resource_type="srq", value="srq")
        )  # SRQ address
...
class OpenQP(VerbCall):
    ...
    def __init__(self, ctx_var=None, qp: str = None, attr_var: str = None, attr_obj: IbvQPOpenAttr = None):
...
class OpenXRCD(VerbCall):
    ...
    def __init__(self, ctx_var: str = None, xrcd: str = None, attr_var: str = None, attr_obj: IbvXRCDInitAttr = None):
        # Context variable name, e.g., "ctx"
        self.ctx_var = ConstantValue(ctx_var or "ctx")
        self.xrcd = (
            ResourceValue(resource_type="xrcd", value=xrcd)
...
class PostRecv(VerbCall):
    ...
    def __init__(self, qp: str = None, wr_obj: IbvRecvWR = None, wr_var: str = None, bad_wr_var: str = None):
        self.qp = (
            ResourceValue(resource_type="qp", value=qp) if qp else ResourceValue(resource_type="qp", value="qp")
        )  # QP address
        self.wr_obj = wr_obj
...
class PostSend(VerbCall):  # TODO: 修改varname
    ...
    def __init__(self, qp: str = None, wr_obj: IbvSendWR = None):
...
class PostSRQRecv(VerbCall):
    ...
    def __init__(self, srq: str = None, wr_obj: IbvRecvWR = None, wr_var: str = None, bad_wr_var: str = None):
        self.srq = (
            ResourceValue(resource_type="srq", value=srq) if srq else ResourceValue(resource_type="srq", value="srq")
        )
        self.wr_obj = wr_obj
...
class QueryDeviceAttr(VerbCall):
    ...
    def __init__(self, output: str = None):
        self.output = ConstantValue(output or "dev_attr")  # Default output variable name
        pass
...
class QueryDeviceEx(VerbCall):
    ...
    def __init__(self, ctx_var: str = None, attr_var: str = None, comp_mask: int = None, input_var: str = None):
...
class QueryECE(VerbCall):
    ...
    def __init__(self, qp: str = None, output: str = None):
        self.qp = ResourceValue(resource_type="qp", value=qp) if qp else ResourceValue(resource_type="qp", value="qp")
        self.output = ConstantValue(output or "ece_options")  # Default output variable name
        self.tracker = None
        self.required_resources = []
...
class QueryGIDEx(VerbCall):
    ...
    def __init__(self, port_num: int = None, gid_index: int = None, flags: int = None, output: str = None):
        self.port_num = IntValue(port_num or 1)  # Default port number
        self.gid_index = IntValue(gid_index or 0)  # Default GID index
        self.flags = IntValue(flags or 0)  # Default flags # now must be 0
        self.output = ConstantValue("gid" or output)  # Variable name for the GID entry output
...
class QueryGIDTable(VerbCall):
    ...
    def __init__(self, max_entries: int = None, output: str = None):
        ...
        self.output = ConstantValue(output or "gid_entries")  # Variable name for the GID entries output
...
class QueryPKey(VerbCall):
    ...
    def __init__(self, port_num: int = None, index: int = None, pkey: str = None):
        ...
        self.pkey = ConstantValue(pkey or "pkey")  # Variable name for the P_Key output
...
class RegDmaBufMR(VerbCall):
    ...
    def __init__(
        ...
        self.fd = IntValue(fd or 0)  # Default file descriptor # 注意这个可能不能变异，得做成fd变量
...
class RegMR(VerbCall):
    ...
    def __init__(self, pd: str = None, mr: str = None, buf: str = None, length: int = None, flags: str = None):
        self.pd = ResourceValue(resource_type="pd", value=pd) if pd else ResourceValue(resource_type="pd", value="pd")
        self.mr = ResourceValue(resource_type="mr", value=mr) if mr else ResourceValue(resource_type="mr", value="mr")
...
class RegMRIova(VerbCall):
    ...
    def __init__(
        self, pd: str = None, mr: str = None, buf: str = None, length: int = None, iova: int = None, access: str = None
    ):
        self.pd = ResourceValue(resource_type="pd", value=pd) if pd else ResourceValue(resource_type="pd", value="pd")
        self.mr = ResourceValue(resource_type="mr", value=mr) if mr else ResourceValue(resource_type="mr", value="mr")
        self.buf = ConstantValue(buf or "buf")  # Default buffer variable name
...
class ReqNotifyCQ(VerbCall):
    ...
    def __init__(self, cq: str = None, solicited_only: int = None):
        self.cq = ResourceValue(resource_type="cq", value=cq) if cq else ResourceValue(resource_type="cq", value="cq")
        self.solicited_only = IntValue(solicited_only or 0)  # Default solicited_only value
        self.tracker = None
        self.required_resources = []
...
class ReRegMR(VerbCall):
    ...
    def __init__(
        ...
        self.addr = ConstantValue(addr or "NULL")  # Default address variable name
        self.length = IntValue(length or 0)  # Default length
        self.access = FlagValue(access or 0, flag_type="IBV_ACCESS_FLAGS_ENUM")  # Default access flags
        self.tracker = None
        self.required_resources = []
...
class SetECE(VerbCall):
    ...
    def __init__(self, qp: str = None, ece_obj: "IbvECE" = None, ece_var: str = None):
        ...
        self.ece_var = ConstantValue(ece_var or "ece")  # Default variable name for the ECE structure
