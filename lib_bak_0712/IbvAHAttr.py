...
class IbvGlobalRoute(Attr):
    ...
    def __init__(self, dgid=None, flow_label=None, sgid_index=None, hop_limit=None, traffic_class=None):
        self.dgid = dgid
        self.flow_label = IntValue(flow_label) # not necessary
        self.sgid_index = IntValue(sgid_index)  # not necessary
        # hop_limit and traffic_class are usually set to 0, but can be mutated
        self.hop_limit = IntValue(hop_limit)
        self.traffic_class = IntValue(traffic_class)
