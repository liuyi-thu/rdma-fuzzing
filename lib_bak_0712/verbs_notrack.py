...
class AdviseMR(VerbCall):
    ...
    def __init__(
        self, pd_addr: str, advice: int, flags: int, sg_list: list, num_sge: int = None, sg_var: str = "sg_list"
    ):
...
class CreateCQ(VerbCall):
    ...
    def from_trace(cls, info: str, ctx: CodeGenContext):
        ...
        return cls(
            context=context, cqe=cqe, cq_context=cq_context, channel=channel, comp_vector=comp_vector, cq_addr=cq_addr
        )
...
class ModifyQP(VerbCall):
    def __init__(
        self, qp_addr: str, attr: IbvQPAttr, attr_mask: str, ctx: CodeGenContext = None
...
class PostRecv(VerbCall):
    ...
    def generate_c(self, ctx: CodeGenContext) -> str:
        ...
        if ctx:
            ctx.alloc_variable(
                self.bad_wr_var, "struct ibv_recv_wr *", "NULL"
...
class PostSend(VerbCall):
    ...
    def from_trace(cls, info, ctx):
        ...
        if "wr_obj" in kv:
            wr_obj = kv["wr_obj"]
        return cls(qp_addr=kv.get("qp"), wr_addr=kv.get("wr"), bad_wr_addr=kv.get("bad_wr"), wr_obj=wr_obj)
    ...
    def generate_c(self, ctx):
        ...
        if bad_wr_name:
            ctx.alloc_variable(
                bad_wr_name, "struct ibv_send_wr *", "NULL"
            )  # Register the bad work request pointer in the context
        # 3. 生成ibv_post_send调用
...
class QueryDeviceEx(VerbCall):
    ...
    def generate_c(self, ctx: CodeGenContext) -> str:
        ctx.alloc_variable(
            self.input_var, "struct ibv_query_device_ex_input"
...
class RegDmaBufMR(VerbCall):
    def __init__(
        self, pd_addr: str, mr_addr: str, offset: int, length: int, iova: int, fd: int, access: int, ctx: CodeGenContext
    ):
        self.pd_addr = pd_addr
        self.mr_addr = mr_addr
        self.offset = offset
...
class ReRegMR(VerbCall):
    ...
    def from_trace(cls, info: str, ctx: CodeGenContext):
        ...
        return cls(mr_addr=mr, flags=flags, pd_addr=pd, addr=addr, length=length, access=access)
