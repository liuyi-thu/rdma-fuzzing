...
class CreateQP(VerbCall):
    ...
    def generate_c(self, ctx: CodeGenContext) -> str:
        ...
        ctx.alloc_varaible(attr_name, "struct ibv_qp_init_attr")  # Register the attribute name in the context
        cap = self.cap_params
        cap_lines = "\n    ".join(
...
class ModifyQP(VerbCall):
    ...
    def generate_c(self, ctx: CodeGenContext) -> str:
        ...
        ctx.alloc_varaible(attr_name, "struct ibv_qp_attr")  # Register the attribute variable in the context
...
class PostSend(VerbCall):
    ...
    def generate_c(self, ctx: CodeGenContext) -> str:
        ...
        ctx.alloc_varaible(sr, "struct ibv_send_wr")  # Register the send request variable in the context
        ctx.alloc_varaible(sge, "struct ibv_sge")  # Register the scatter/gather entry variable in the context
        ctx.alloc_varaible(bad_wr, "struct ibv_send_wr *")  # Register the bad work request pointer in the context
