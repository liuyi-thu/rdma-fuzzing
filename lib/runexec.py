...
# TRADITIONAL FEEDBACK - COMMENTED OUT (imports only used in commented code)
# import json
# import os.path
# import re
...
# TRADITIONAL FEEDBACK - COMMENTED OUT
...
def build_and_run() -> Dict[str, Any]:
    ...
    # Return data without traditional feedback metrics
    # dmesg is still collected for kernel crash detection
    return {
        "outcome": "ok",
        ...
        "sem_signature": set(),
        "crash_site": None,
        "dmesg_new": dmesg_content,  # 保留dmesg用于内核崩溃检测
    }
...
def execute_and_collect() -> Dict[str, Any]:
    ...
    # Dummy values when traditional feedback is disabled
    cov_new = 0
    sem_new = 0
    score = 0.0
    keep = False
    print("[+] Traditional feedback disabled, using dummy values")
